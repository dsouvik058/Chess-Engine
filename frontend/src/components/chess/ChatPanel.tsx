...
export const ChatPanel: React.FC<ChatPanelProps> = ({
  ...
  return (
    <Card className="flex flex-col h-64 p-3 bg-slate-950/80 border-slate-800">
      <div className="flex items-center gap-2 pb-2 mb-2 border-b border-slate-800 text-xs font-bold text-slate-300">
        <MessageSquare className="w-4 h-4 text-cyan-400" />
        <span>Opponent Chat</span>
      </div>

      {/* Message List */}
      <div ref={logRef} className="flex-1 overflow-y-auto space-y-2 pr-1 text-xs">
        {messages.length === 0 ? (
          <div className="text-center py-6 text-slate-500 italic">Connected to 1v1 Room. Say hi to your opponent!</div>
        ) : (
      ...
      {/* Input Row */}
      <form onSubmit={handleSubmit} className="flex gap-2 mt-2 pt-2 border-t border-slate-800">
        <input
          type="text"
