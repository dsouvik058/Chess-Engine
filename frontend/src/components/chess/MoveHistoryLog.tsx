...
export const MoveHistoryLog: React.FC<MoveHistoryLogProps> = ({
  ...
  return (
    <div className="flex flex-col h-64 sm:h-72 glass-card rounded-2xl border border-slate-200 p-3.5 overflow-hidden shadow-xl shadow-slate-200/50 bg-white/90">
      {/* Header */}
      <div className="flex items-center justify-between pb-2.5 mb-2 border-b border-slate-200 text-xs font-bold text-slate-800 font-serif-classic">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto space-y-1 pr-1 font-mono text-xs scroll-smooth">
        {moves.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-slate-400 italic text-xs space-y-1">
            ...
            <span className="text-[10px] text-slate-400">Make a move on the board to start log</span>
          </div>
        ) : (
          <AnimatePresence initial={false}>
            {moves.map((item, idx) => (
              <motion.div
                ...
                className={`grid grid-cols-12 gap-2 items-center py-1.5 px-2 rounded-xl transition-colors ${
                  idx % 2 === 0 ? 'bg-amber-50/40' : 'bg-transparent'
              ...
              >
                ...
                <motion.button
                  ...
                  className="col-span-5 flex items-center gap-1 text-left text-slate-800 hover:text-amber-900 font-bold transition-colors cursor-pointer"
                >
